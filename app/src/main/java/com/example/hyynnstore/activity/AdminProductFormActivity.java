...
public class AdminProductFormActivity extends Activity {
    ...
    private int id;
    private EditText name, price, salePrice, image, des, cate, brand, spec, stock;
    ...
    @Override protected void onCreate(Bundle b) {
        ...
        cate = Ui.input(this, "Danh mục sản phẩm (nhập số 1-8)");
        brand = Ui.input(this, "Thương hiệu");
        spec = Ui.input(this, "Thông số");
        stock = Ui.input(this, "Tồn kho");
        addInput(root, name); addInput(root, price); addInput(root, salePrice); addInput(root, image); addInput(root, des); addInput(root, cate); addInput(root, brand); addInput(root, spec); addInput(root, stock);
        ...
        if (p != null) {
            ...
            if (selectedImageUri != null && !selectedImageUri.trim().isEmpty()) { for (String part : selectedImageUri.split("\\|\\|")) selectedImages.add(part); }
            des.setText(p.description);
            cate.setText(String.valueOf(p.categoryId));
    ...
    private void addInput(LinearLayout root, EditText editText) {
        root.addView(editText, new LinearLayout.LayoutParams(-1, Ui.dp(this, 48)));
        Ui.gap(this, root, 8);
    }
    ...
    private void showAssetImagePicker() {
        try {
            ...
            for (int i = 0; i < files.length; i++) {
                imagePaths[i] = "asset://product_images/" + files[i];
            }
            ...
            for (String path : imagePaths) {
                ...
                assetPreview.setScaleType(ImageView.ScaleType.CENTER_CROP);
    ...
    private void renderPreview(String value) {
        ...
        if (value != null && value.startsWith("asset://")) {
            loadAssetInto(preview, value);
        } else if (value != null && (value.startsWith("content://") || value.startsWith("file://"))) {
            preview.setImageURI(Uri.parse(value));
        } else {
    ...
    private String joinImages() {
        ...
        for (String img : selectedImages) { if (img != null && !img.trim().isEmpty()) { if (b.length() > 0) b.append("||"); b.append(img); } }
        return b.toString();
    }
    ...
    private void save() {
        try {
            ...
            Product p = new Product(
                    ...
                    Integer.parseInt(cate.getText().toString().trim()),
